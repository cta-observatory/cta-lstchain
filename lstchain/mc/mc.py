...
__all__ = ["power_law_integrated_distribution", "int_diff_sp", "rate", "weight"]


def power_law_integrated_distribution(
    emin, emax, tot_num_events, spectral_index, bin_number=30
):
    """
    ...
    if spectral_index == -1:
        ...
        y0 = (
            tot_num_events
            / (emax ** (spectral_index + 1) - emin ** (spectral_index + 1))
            * (spectral_index + 1)
        )

        y = (
            y0
            * (bins[1:] ** (spectral_index + 1) - bins[:-1] ** (spectral_index + 1))
            / (spectral_index + 1)
        )
    return bins, y
...
def int_diff_sp(emin, emax, sp_idx, e0):
    ...
    if sp_idx == -1:
        ...
        integral_e = (
            (emax ** (sp_idx + 1) - emin ** (sp_idx + 1)) / (sp_idx + 1) / e0 ** sp_idx
        )
...
def weight(shape, emin, emax, sim_sp_idx, rate, nev, w_param):
    ...
    if shape == "PowerLaw":
        ...
        norm_sim = sim_integral * int_diff_sp(
            emin, emax, w_param["alpha"], w_param["e0"]
        )
