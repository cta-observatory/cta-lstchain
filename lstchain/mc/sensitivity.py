...
def find_cut_real(events_on, events_off, obstime_on, obstime_off, feature, low_cut, high_cut, gamma_efficiency):
    ...
    while tol > 1e-6:
        ...
        if samesign(diff_events_after_cut_real(events_on, events_off, obstime_on, obstime_off, feature, lookfor_cut,
                                               gamma_efficiency),
                    diff_events_after_cut_real(events_on, events_off, obstime_on, obstime_off, feature, midpoint,
                                               gamma_efficiency)):
            lookfor_cut = midpoint
        else:
            alternative_cut = midpoint
...
def sensitivity_gamma_efficiency(dl2_file_g, dl2_file_p,
    ...
    for i in range(0, n_bins_energy):  # binning in energy
        ...
        events_bin_after_cuts_g = events_bin_g[
            (events_bin_g.gammaness > best_g_cut) & (events_bin_g.theta2 < best_theta2_cut)]
        events_bin_after_cuts_p = events_bin_p[
            (events_bin_p.gammaness > best_g_cut) & (events_bin_p.theta2 < best_theta2_cut)]
...
def sensitivity_gamma_efficiency_real_protons(dl2_file_g, dl2_file_p,
    ...
    for i in range(0, n_bins_energy):  # binning in energy
        ...
        best_theta2_cut = find_cut(events_g_after_g_cut, rates_g_after_g_cut, obstime, "theta2", 0.0, .5,
