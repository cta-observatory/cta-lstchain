...
def diff_events_after_cut_real(
    events_on, events_off, obstime_on, obstime_off, feature, cut, gamma_efficiency
):
    ...
    if feature == "gammaness":
        ...
        events_off_after_cut = (
            events_off[events_off[feature] > cut].shape[0] * obstime_on / obstime_off
        )
...
def find_cut(events, rates, obstime, feature, low_cut, high_cut, gamma_efficiency):
    ...
    while tol > 1e-6:
        ...
        if samesign(
            diff_events_after_cut(
                events, rates, obstime, feature, lookfor_cut, gamma_efficiency
            ),
            diff_events_after_cut(
                events, rates, obstime, feature, midpoint, gamma_efficiency
            ),
        ):
...
def calculate_sensitivity_lima(n_signal, n_background, alpha):
    ...
    stat = WStatCountsStatistic(
        n_on=n_signal + alpha * n_background, n_off=n_background, alpha=alpha
    )
...
def calculate_sensitivity_lima_ebin(n_on_events, n_background, alpha, n_bins_energy):
    ...
    stat = WStatCountsStatistic(n_on=n_on_events, n_off=n_background, alpha=alpha)
...
def ring_containment(angdist2, ring_radius, ring_halfwidth):
    ...
    contained = np.where(
        (np.sqrt(angdist2) < ring_upper_limit) & (np.sqrt(angdist2) > ring_lower_limit),
        True,
        False,
    )
...
def sensitivity_gamma_efficiency(
    dl2_file_g,
    dl2_file_p,
    ntelescopes_gammas,
    ntelescopes_protons,
    ...
    energy = (
        np.logspace(
            np.log10(emin_sensitivity.to_value()),
    ...
    rate_weighted_g = (
        (e_true_g / crab_par["e0"]) ** (crab_par["alpha"] - mc_par_g["sp_idx"])
    ) * w_g
    rate_weighted_p = (
        (e_true_p / proton_par["e0"]) ** (proton_par["alpha"] - mc_par_p["sp_idx"])
    ) * w_p
    ...
    for i in range(0, n_bins_energy):  # binning in energy
        ...
        total_rate_proton_ebin = np.sum(
            rate_weighted_p[(e_reco_p < energy[i + 1]) & (e_reco_p > energy[i])]
        )
        total_rate_gamma_ebin = np.sum(
            rate_weighted_g[(e_reco_g < energy[i + 1]) & (e_reco_g > energy[i])]
        )

        # print("**************")
        print(
            "Total rate triggered proton in this bin {:.5f} Hz".format(
                total_rate_proton_ebin.value
            )
        )
        print(
            "Total rate triggered gamma in this bin {:.5f} Hz".format(
        ...
        best_g_cut = find_cut(
            events_bin_g, rates_g, obstime, "gammaness", 0.1, 1.0, gamma_eff_gammaness
        )
        best_theta2_cut = (
            find_cut(
                events_bin_g, rates_g, obstime, "theta2", 0.0, 10.0, gamma_eff_theta2
            )
            * u.deg ** 2
        )
        ...
        rate_g_ebin = np.sum(
            rate_weighted_g[
                (e_reco_g < energy[i + 1])
                & (e_reco_g > energy[i])
                & (gammaness_g > best_g_cut)
                & (theta2_g < best_theta2_cut)
            ]
        )
        ...
        pre_gammas[i] = e_reco_g[
            (e_reco_g < energy[i + 1])
            & (e_reco_g > energy[i])
            & (gammaness_g > best_g_cut)
            & (theta2_g < best_theta2_cut)
        ].shape[0]
        pre_protons[i] = e_reco_p[
            (e_reco_p < energy[i + 1])
            & (e_reco_p > energy[i])
            & (gammaness_p > best_g_cut)
            & p_contained
        ...
        weighted_gamma_per_ebin[i] = (
            np.sum(rate_weighted_g[(e_reco_g < energy[i + 1]) & (e_reco_g > energy[i])])
            * obstime
        )
        weighted_proton_per_ebin[i] = (
            np.sum(rate_weighted_p[(e_reco_p < energy[i + 1]) & (e_reco_p > energy[i])])
            * obstime
        )
    ...
    n_excesses_min, sensitivity = calculate_sensitivity_lima(
        final_gammas, final_protons * noff, 1 / noff * np.ones_like(final_gammas)
    )
    ...
    sensitivity_flux = (
        sensitivity / 100 * (dFdE * egeom * egeom).to(u.TeV / (u.cm ** 2 * u.s))
    )
    ...
    list_of_tuples = list(
        zip(
            energy[: energy.shape[0] - 1].to_value(),
            energy[1:].to_value(),
            gcut,
            tcut,
            final_gammas,
...
def sensitivity_gamma_efficiency_real_protons(
    dl2_file_g,
    dl2_file_p,
    ntelescopes_gammas,
    n_bins_energy,
    ...
    for i in range(0, n_bins_energy):  # binning in energy
        ...
        total_rate_proton_ebin = (
            e_reco_p[(e_reco_p < energy[i + 1]) & (e_reco_p > energy[i])].shape[0]
            / obstime_real
        )
        total_rate_gamma_ebin = np.sum(
            rate_weighted_g[(e_reco_g < energy[i + 1]) & (e_reco_g > energy[i])]
        ...
        best_g_cut = find_cut(
            events_bin_g, rates_g, obstime, "gammaness", 0.0, 1.0, gamma_eff_gammaness
        )
        ...
        events_bin_after_cuts_p = events_p[
            (e_reco_p < energy[i + 1])
            & (e_reco_p > energy[i])
            & (gammaness_p > best_g_cut)
            & p_contained
...
def sensitivity_gamma_efficiency_real_data(
    dl2_file_on,
    dl2_file_off,
    gcut,
    tcut,
    ...
    for i in range(0, n_bins_energy):  # binning in energy
        ...
        total_rate_off_ebin = (
            e_reco_off[(e_reco_off < energy[i + 1]) & (e_reco_off > energy[i])].shape[0]
            / obstime_off
        )
        total_rate_on_ebin = (
            e_reco_on[(e_reco_on < energy[i + 1]) & (e_reco_on > energy[i])].shape[0]
            / obstime_on
        )

        # print("**************")
        print(
            "Total rate triggered off events in this bin {:.5f} Hz".format(
                total_rate_off_ebin.value
            )
        )
        print(
            "Total rate triggered on events in this bin {:.5f} Hz".format(
        ...
        events_bin_off = events_off[
            (e_reco_off < energy[i + 1]) & (e_reco_off > energy[i])
        ]

        best_g_cut = gcut[
            i
        ]  # find_cut(events_bin_on, 1, obstime,  "gammaness", 0, 1.0, gamma_eff_gammaness, True)
        ...
        best_theta2_cut = tcut[
            i
        ]  # find_cut_real(events_on_after_g_cut, events_off_after_g_cut, obstime_on, obstime_off, "theta2", 0.0, 1.0, gamma_eff_theta2) * u.deg**2
        # tcut[i]=best_theta2_cut.to_value()
        best_theta2_cut_off = 0.5  # * u.deg**2

        events_bin_after_cuts_on = events_bin_on[
            (events_bin_on.gammaness > best_g_cut)
            & (events_bin_on.theta2 < best_theta2_cut)
        ]

        events_bin_after_cuts_off = events_bin_off[
            (events_bin_off.gammaness > best_g_cut)
