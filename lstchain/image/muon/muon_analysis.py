...
def pixel_coords_to_telescope(geom, equivalent_focal_length):
    ...
    camera_coord = SkyCoord(
        geom.pix_x,
        geom.pix_y,
        CameraFrame(focal_length=equivalent_focal_length, rotation=geom.cam_rotation),
    )
...
def analyze_muon_event(
    ...
    cam_rad = (
        lst1_description.camera.geometry.guess_radius()
        / lst1_description.optics.equivalent_focal_length
    ) * u.rad

    # some cuts for good ring selection:
    min_pix = 148  # (8%) minimum number of pixels in the ring with >0 signal
    min_pix_fraction_after_cleaning = (
        0.1  # minimum fraction of the ring pixels that must be above tailcuts[0]
    )
    min_ring_radius = 0.8 * u.deg  # minimum ring radius
    max_ring_radius = 1.5 * u.deg  # maximum ring radius
    max_radial_stdev = (
        0.1 * u.deg
    )  # maximum standard deviation of the light distribution along ring radius
    max_radial_excess_kurtosis = 1.0  # maximum excess kurtosis
    ...
    dist_mask_2 = np.logical_and(
        ~dist_mask,
        np.abs(dist - muonringparam.radius)
        < muonringparam.radius * (ring_integration_width + outer_ring_width),
    )
    ...
    candidate_clean_ring = all(
        [
            ...
            (pix_ring > tailcuts[0]).sum() > min_pix_fraction_after_cleaning * min_pix,
    ...
    if candidate_clean_ring:
        ...
        dist_ringwidth_mask = (
            np.abs(dist - muonringparam.radius) < muonintensityoutput.width
        )
    ...
    if plot_rings and plots_path and good_ring:
        ...
        ringrad_inner = ringrad_camcoord * (1.0 - ringwidthfrac)
        ringrad_outer = ringrad_camcoord * (1.0 + ringwidthfrac)
        ...
        plt.figtext(
            0.15,
            0.18,
            "radial excess kurtosis: {0:.3f}".format(
                radial_distribution["excess_kurtosis"]
            ),
        )
        plt.figtext(0.15, 0.16, "fitted ring width: {0:.3f}".format(width))
        plt.figtext(
            0.15, 0.14, "ring completeness: {0:.3f}".format(muonparameters.completeness)
        )
...
def muon_filter(image, thr_low=0, thr_up=1.0e10):
    """
...
def radial_light_distribution(center_x, center_y, pixel_x, pixel_y, image):
    ...
    return {
        "standard_dev": standard_dev * u.deg,
        "skewness": skewness,
