...
def train_sep(train, custom_config={}):
    ...
    clf.fit(train[features], train["mc_type"])
    print("Random Forest trained!")
    return clf
...
def build_models(
    ...
    if config["source_dependent"]:
        ...
        src_dep_df_gamma.columns = pd.MultiIndex.from_tuples(
            [
                ...
                for col in src_dep_df_gamma.columns
            ]
        )
        ...
        src_dep_df_proton.columns = pd.MultiIndex.from_tuples(
            [
                tuple(col[1:-1].replace("'", "").replace(" ", "").split(","))
                for col in src_dep_df_proton.columns
            ]
        )
    ...
    test["log_reco_energy"] = temp_reg_energy.predict(
        test[config["regression_features"]]
    )
...
def apply_models(
    ...
    if "mc_alt_tel" in dl2.columns:
        ...
        alt_tel = -np.pi / 2.0 * np.ones(len(dl2))
        az_tel = -np.pi / 2.0 * np.ones(len(dl2))
...
def get_source_dependent_parameters(data, config, focal_length=28 * u.m):
    ...
    expected_src_pos_x_m, expected_src_pos_y_m = get_expected_source_pos(
        data, data_type, config, focal_length=focal_length
    )

    # ON position
    src_dep_params_dict = {}
    src_dep_params = calc_source_dependent_parameters(
        data, expected_src_pos_x_m, expected_src_pos_y_m
    )
    ...
    if not is_simu:
        if config.get("observation_mode") == "wobble":
            for ioff in range(config.get("n_off_wobble")):
                ...
                rotated_expected_src_pos_x_m = expected_src_pos_x_m * np.cos(
                    off_angle
                ) - expected_src_pos_y_m * np.sin(off_angle)
                rotated_expected_src_pos_y_m = expected_src_pos_x_m * np.sin(
                    off_angle
                ) + expected_src_pos_y_m * np.cos(off_angle)
                src_dep_params = calc_source_dependent_parameters(
                    data, rotated_expected_src_pos_x_m, rotated_expected_src_pos_y_m
                )
...
def calc_source_dependent_parameters(data, expected_src_pos_x_m, expected_src_pos_y_m):
    ...
    src_dep_params["dist"] = np.sqrt(
        (data["x"] - expected_src_pos_x_m) ** 2
        + (data["y"] - expected_src_pos_y_m) ** 2
    )

    disp, miss = camera_to_shower_coordinates(
        expected_src_pos_x_m, expected_src_pos_y_m, data["x"], data["y"], data["psi"]
    )

    src_dep_params["time_gradient_from_source"] = (
        data["time_gradient"] * np.sign(disp) * -1
    )
...
def get_expected_source_pos(data, data_type, config, focal_length=28 * u.m):
    ...
    if data_type == "mc_proton":
        expected_src_pos = utils.sky_to_camera(
            u.Quantity(
                data["mc_alt_tel"].values + config["mc_nominal_source_x_deg"],
                u.deg,
                copy=False,
            ),
            u.Quantity(
                data["mc_az_tel"].values + config["mc_nominal_source_y_deg"],
                u.deg,
                copy=False,
            ),
            focal_length,
    ...
    if data_type == "real_data":
        ...
        if config.get("observation_mode") == "wobble":
            ...
            source_pos = utils.radec_to_camera(
                source_coord, obstime, pointing_alt, pointing_az, focal_length
            )
