...
class DL1DataCheckContainer(Container):
    ...
    hist_delta_t = Field(
        None, "Histogram of time difference between " "consecutive events"
    )
    ...
    hist_dist0_intensity_gt_200 = Field(None, "Histogram of cog-camera center distance")
    ...
    hist_intercept = Field(None, "Histogram of fitted pulse time for charge " "c.o.g.")
    hist_tgrad_vs_length = Field(None, "Histogram of time gradient vs. length")
    hist_tgrad_vs_length_intensity_gt_200 = Field(
        None, "Histogram of time gradient vs. length, intensity>200pe"
    )
    ...
    cog_within_pixel_intensity_gt_200 = Field(
        None, "Number of image within pixel, intensity>200pe"
    )
    ...
    time_stddev_above_030_pe = Field(
        -1, "Standard deviaton of pulse time, " ">30 p.e. pulses"
    )
    relative_time_mean = Field(
        -1, "Mean of pulse time relative to average of " "rest of pixels"
    )
    relative_time_stddev = Field(
        -1, "Standard deviaton of pulse time " "relative to average of rest of pixels"
    )
    ...
    def fill_event_wise_info(self, subrun_index, table, mask, geom, histogram_binnings):
        """
        ...
        # events!)
        self.elapsed_time = (
            table["dragon_time"][len(table) - 1] - table["dragon_time"][0]
        )
        self.num_events = mask.sum()
        self.num_cleaned_events = np.isfinite(table["intensity"][mask]).sum()
        self.ucts_trigger_type = count_trig_types(table["ucts_trigger_type"][mask])
        self.trigger_type = count_trig_types(table["trigger_type"][mask])
        ...
        tib_time = u.Quantity(
            np.array(table["tib_time"][mask][0::n_jump]), u.s, copy=False
        )
        ucts_time = u.Quantity(
            np.array(table["ucts_time"][mask][0::n_jump]), u.s, copy=False
        )
        dragon_time = u.Quantity(
            np.array(table["dragon_time"][mask][0::n_jump]), u.s, copy=False
        )
        ...
        delta_t = np.array(table["dragon_time"][1:]) - np.array(
            table["dragon_time"][:-1]
        )
        (
            counts,
            _,
            _,
        ) = plt.hist(delta_t * 1.0e3, bins=histogram_binnings.hist_delta_t)
        ...
        (
            ...
            _,
            _,
        ) = plt.hist(n_pixels, bins=histogram_binnings.hist_npixels)
        ...
        (
            ...
            _,
            _,
        ) = plt.hist(n_islands, bins=histogram_binnings.hist_nislands)
        self.hist_nislands = counts

        intensity = table.loc[mask, "intensity"].to_numpy()
        counts, _, _ = plt.hist(intensity, bins=histogram_binnings.hist_intensity)
        ...
        counts, _, _ = plt.hist(
            dist0[intensity > 200], bins=histogram_binnings.hist_dist0_intensity_gt_200
        )
        ...
        counts, _, _ = plt.hist(psi, bins=histogram_binnings.hist_psi)
        self.hist_psi = counts

        counts, _, _, _ = plt.hist2d(
            intensity,
            table.loc[mask, "intercept"].to_numpy(),
        ...
        counts, _, _, _ = plt.hist2d(
            length, tgrad, bins=histogram_binnings.hist_tgrad_vs_length
        )
        ...
        counts, _, _, _ = plt.hist2d(
            length[intensity > 200],
            tgrad[intensity > 200],
            bins=histogram_binnings.hist_tgrad_vs_length_intensity_gt_200,
        )
        ...
        cog_pixid = geom.position_to_pix_index(np.array(x) * u.m, np.array(y) * u.m)
    ...
    def fill_pixel_wise_info(self, table, mask, histogram_binnings, event_type=""):
        """
        ...
        with warnings.catch_warnings():
            ...
            n_valid_pixels = np.array(
                [np.sum([~np.isnan(row)]) for row in selected_entries]
            )
            ...
            # time:
            camera_valid_pixels = np.tile(
                n_valid_pixels, (time.shape[1], 1)
            ).transpose()
            rest_of_camera_valid_pixels = camera_valid_pixels - np.ones(
                camera_valid_pixels.shape
            )
            mean_t_of_rest_of_camera = (
                camera_time_mean * camera_valid_pixels - time
...
class DL1DataCheckHistogramBins(Container):
    ...
    hist_delta_t = Field(np.linspace(-1.0e-2, 2.0, 200), "hist_delta_t binning")
    # pixel charge and image intensity (units: p.e):
    hist_pixelchargespectrum = Field(
        np.logspace(-1.0, 4.7, 121), "hist_pixelchargespectrum binning"
    )
    hist_intensity = Field(np.logspace(1.0, 6.0, 101), "hist_intensity binning")

    # dist0, width and length (units: degrees):
    hist_dist0 = Field(np.linspace(0.0, 2.5, 50), "hist_dist0 binning")
    hist_dist0_intensity_gt_200 = Field(
        np.linspace(0.0, 2.5, 50), "hist_dist0_intensity_gt_200 binning"
    )
    hist_psi = Field(np.linspace(-100.0, 100.0, 101), "hist_psi binning")
    hist_psi_intensity_gt_200 = Field(
        np.linspace(-100.0, 100.0, 101), "hist_psi_intensity_gt_200 binning"
    )
    ...
    hist_width = Field(
        np.array([np.logspace(0.7, 5.7, 101), np.linspace(0.0, 0.8, 101)]),
        "hist_width binning",
    )
    hist_length = Field(
        np.array([np.logspace(0.7, 5.7, 101), np.linspace(0.0, 1.0, 101)]),
        "hist_length binning",
    )
    hist_skewness = Field(
        np.array([np.logspace(0.7, 5.7, 101), np.linspace(-4.0, 4.0, 101)]),
        "hist_skewness binning",
    )
    # time gradient vs. length:
    hist_tgrad_vs_length = Field(
        np.array([np.linspace(0.0, 1.0, 101), np.linspace(0.0, 200.0, 101)]),
        "hist_tgrad_vs_length binning",
    )
    hist_tgrad_vs_length_intensity_gt_200 = Field(
        np.array([np.linspace(0.0, 1.0, 101), np.linspace(0.0, 50.0, 101)]),
    ...
    hist_intercept = Field(
        np.array([np.logspace(0.7, 5.7, 101), np.linspace(-30.0, 40.0, 101)]),
