...
def check_dl1(filenames, output_path, max_cores=4, create_pdf=False, batch=False):
    ...
    if len(filenames) == 1:
        datacheck_filename = run_to_datacheck_dl1_filename(
            first_file.tel_id, first_file.run, first_file.subrun
        )
    else:
        datacheck_filename = run_to_datacheck_dl1_filename(
            first_file.tel_id, first_file.run, None
        )
    ...
    for filename in filenames:
        with tables.open_file(
            filename, root_uep="/dl1/event/telescope/parameters"
        ) as f:
            for name in trig_tags:
                trig_tags[name].extend(f.root.LST_LSTCam.col(name))
    num_pedestals = {
        "trigger_type": (np.array(trig_tags["trigger_type"]) == 32).sum(),
        "ucts_trigger_type": (np.array(trig_tags["ucts_trigger_type"]) == 32).sum(),
    }
    ...
    writer_conf = tables.Filters(complevel=9, complib="blosc:zstd", fletcher32=True)
    ...
    file.create_dataset(
        "/dl1datacheck/used_trigger_tag", (1,), "S32", [trigger_source.encode("ascii")]
    )
    file.close()
...
def process_dl1_file(filename, bins, tel_id=1):
    ...
    with tables.open_file(filename) as file:
        ...
        pedestal_mask = parameters["event_type"] == EventType.SKY_PEDESTAL.value

        # Now obtain by exclusion the masks for cosmics:
        cosmics_mask = ~(pedestal_mask | flatfield_mask)

        logger.info(
            f"   pedestals: {np.sum(pedestal_mask)}, "
        ...
        if pedestal_mask.sum() > 1:
            dl1datacheck_pedestals.fill_event_wise_info(
                subrun_index, parameters, pedestal_mask, geom, bins
            )
            dl1datacheck_pedestals.fill_pixel_wise_info(
                image_table, pedestal_mask, bins, "pedestals"
            )
        else:
            dl1datacheck_pedestals = None

        if flatfield_mask.sum() > 1:
            dl1datacheck_flatfield.fill_event_wise_info(
                subrun_index, parameters, flatfield_mask, geom, bins
            )
            dl1datacheck_flatfield.fill_pixel_wise_info(
                image_table, flatfield_mask, bins, "flatfield"
            )
        else:
            dl1datacheck_flatfield = None

        if cosmics_mask.sum() > 1:
            dl1datacheck_cosmics.fill_event_wise_info(
                subrun_index, parameters, cosmics_mask, geom, bins
            )
            dl1datacheck_cosmics.fill_pixel_wise_info(
                image_table, cosmics_mask, bins, "cosmics"
            )
        else:
...
def plot_datacheck(
    datacheck_filename, out_path=None, batch=False, muons_dir=None, tel_id=1
):
    """
    ...
    with PdfPages(pdf_filename) as pdf:
        ...
        plt.text(
            0.1,
            0.7,
            os.path.basename(datacheck_filename),
            fontsize=32,
        ...
        plt.text(
            ...
            0.5,
            "    (from Dragon time): "
            + str(datetime.utcfromtimestamp(table_cosmics.col("dragon_time")[0][0])),
            fontsize=24,
            horizontalalignment="left",
            verticalalignment="center",
        )
        ...
        for table, label in zip(dl1dcheck_tables, labels):
            ...
            axes[1, 1].plot(
                table.col("subrun_index"),
                table.col("num_cleaned_events") / table.col("num_events"),
                fmt,
                label=label,
            )
        ...
        axes[0, 0].plot(
            table_cosmics.col("sampled_event_ids").flatten(),
        ...
        axes[1, 0].plot(np.rad2deg(table_cosmics.col("mean_az_tel")), alt_deg, fmt)
        ...
        mpl_times = np.array(
            [dates.date2num(datetime.utcfromtimestamp(x)) for x in mean_dragon_time]
        )
        axes[1, 1].plot_date(mpl_times, alt_deg, fmt=fmt, xdate=True, tz="utc")
        ...
        colnames = [
            name
            for name in table_cosmics.colnames
            if name.find("num_pulses_above") == 0
        ]
        threshold = [
            int(name[name.find("above_") + 6 : name.find("_pe")]) for name in colnames
        ]

        for table, tname in zip(
            [table_pedestals, table_cosmics], ["pedestals", "cosmics"]
        ):
            ...
            fig.suptitle(
                table.name.upper() + ", relative frequency of pixel charges",
                fontsize="xx-large",
            )
            ...
            pix_events = [np.sum(table.col(colname), axis=0) for colname in colnames]
            ...
            for i, frac in enumerate(fraction):
                zscale = "log" if threshold[i] < 200 and frac.sum() > 0 else "lin"
                cam = CameraDisplay(
                    engineering_geom,
                    frac,
                    ax=axes.flatten()[i],
                    norm=zscale,
                    title="Fraction of >" + str(threshold[i]) + " p.e. pulses",
                )
            ...
            for y in fraction_transposed:
                if y.sum() > 0:
                    axes[1, 2].plot(threshold, y, "o", fillstyle="none", alpha=0.2)
            ...
            for i, y in enumerate(
                ["num_pulses_above_0010_pe", "num_pulses_above_0030_pe"]
            ):
                if np.mean(table.col(y), axis=1).max() > 0:
                    axesb[i].set_yscale("log")
                axesb[i].plot(
                    table.col("subrun_index"),
                    np.mean(table.col(y), axis=1) / table.col("num_events"),
                    fmt,
                )
                axesb[i].set_xlabel("subrun index")
                axesb[i].set_ylim(top=1.0)
        ...
        if table_flatfield is None or len(table_flatfield) == 0:
            ...
            plot_mean_and_stddev(
                table_flatfield,
                engineering_geom,
                ["relative_time_mean", "relative_time_stddev"],
                [
                    "Flat-field mean time (ns)",
                    "Flat-field time std dev (ns)",
                    "FLATFIELD, pixel-wise pulse time relative " "to camera mean",
                ],
                pagesize,
                batch,
            )
        ...
        plot_mean_and_stddev(
            ...
            [
                ...
                "COSMICS, pixel-wise pulse time info for pixel " "charge > 1 p.e.",
            ],
            pagesize,
            batch,
        )
        ...
        for i, item in enumerate(items):
            ...
            nbins = 1001
            epb = (
                events_per_pix[pix_inside & gt0].max()
                - events_per_pix[pix_inside & gt0].min()
            ) / (nbins - 1)
            epb = int(epb + 1.0)
            ...
            _, bins, _ = axes[i, 2].hist(
                event_fraction[pix_inside & gt0], bins=np.linspace(xmin, xmax, nbins)
            )
            ...
            poiss = np.array(
                [
                    poisson.cdf(x2 * all_events, mu) - poisson.cdf(x1 * all_events, mu)
        ...
        fig.tight_layout(rect=[0.05, 0.05, 1.0, 0.9], pad=0.0, h_pad=3.0, w_pad=2.0)
        ...
        for i, hist in enumerate(histos):
            ...
            axes[i, 0].hist(
                bins[:-1],
                bins,
                weights=np.sum(table_cosmics.col(hist), axis=0) / ringarea.value,
                histtype="step",
            )
        ...
        for i, hist in enumerate(histos):
            ...
            _, _, _, image = axes[i, 1].hist2d(
                x, y, bins=bins, weights=contents, norm=colors.LogNorm()
            )
        ...
        for i, hist in enumerate(histos):
            ...
            y = np.array([yy for __ in bins[0][:-1] for yy in bins[1][:-1]])
            contents = np.sum(table_cosmics.col(hist), axis=0).flatten()
            _, _, _, image = axes.flatten()[i].hist2d(
                x, y, bins=bins, weights=contents, norm=colors.LogNorm()
            )
        ...
        dcfile = parse_datacheck_dl1_filename(os.path.basename(datacheck_filename))
        for subrun in subrun_list:
            ...
            name = run_to_muon_filename(dcfile.tel_id, dcfile.run, subrun, None, False)
            ...
            if filename.exists():
                ...
                if len(tcont) > 0:
                    ...
                    logger.warning(f"File {str(filename)} has no valid muon " f"rings!")
            else:
                logger.warning(
                    f"File {str(filename)} not found. "
        ...
        fig.tight_layout(rect=[0.0, 0.0, 1.0, 0.97], pad=3.0, h_pad=3.0, w_pad=2.0)
        fmt = "-"
        ...
        axes[0, 0].plot(subrun_list, num_contained_rings, fmt, label="contained rings")
        ...
        axes[1, 1].hist(
            contained_muons["ring_completeness"],
            bins=binning,
            histtype="step",
        ...
        axes[0, 0].hist(
            np.sqrt(
                contained_muons["ring_center_x"] ** 2.0
                + contained_muons["ring_center_y"] ** 2.0
            ),
        ...
        axes[1, 0].plot(
            contained_muons["impact_parameter"],
            contained_muons["ring_completeness"],
            "x",
            alpha=0.5,
        )
        ...
        axes[0, 1].plot(
            contained_muons["ring_radius"], contained_muons["ring_size"], "x", alpha=0.5
        )
        axes[0, 1].set_xlabel("ring radius (deg)")
        axes[0, 1].set_ylabel("ring intensity (p.e.)")
        axes[1, 1].plot(
            contained_muons["ring_radius"],
            contained_muons["ring_width"],
            "x",
            alpha=0.5,
        )
        ...
        axes[0, 2].hist(contained_muons["ring_size"], bins=np.linspace(0.0, 4.0e3, 41))
        axes[0, 2].set_xlabel("ring intensity (p.e.)")
        axes[0, 2].set_ylabel("number of rings")
        axes[1, 2].hist(contained_muons["ring_width"], bins=np.linspace(0.0, 0.3, 61))
        ...
        axes[0, 0].hist(
            contained_muons["hg_peak_sample"], bins=binning, histtype="step"
        )
        ...
        axes[1, 0].plot(
            contained_muons["ring_completeness"],
            contained_muons["ring_size"] / contained_muons["ring_radius"],
            "x",
            alpha=0.5,
        )
        ...
        axes[0, 1].hist(
            contained_muons["muon_efficiency"], bins=np.linspace(0.0, 0.5, 51)
        )
        ...
        axes[1, 1].plot(
            contained_muons["ring_width"],
            contained_muons["muon_efficiency"],
            "x",
            alpha=0.5,
        )
        ...
        axes[0, 2].errorbar(
            good_subruns, mean_effi, yerr=sem_effi, fmt="o", markersize=3.0
        )
...
def plot_trigger_types(dchecktables, trigger_name, axes):
    ...
    num_triggers = np.array([(tt[:, 1][tt[:, 0] == trig]).sum() for trig in trig_types])
...
def plot_mean_and_stddev(
    table, camgeom, columns, labels, pagesize, batch=False, norm="lin"
):
    """
    Parameters
    ----------
    ...
    cam = CameraDisplay(camgeom, mean, ax=axes[0, 0], norm=norm, title=labels[0])
    cam.add_colorbar(ax=axes[0, 0])
    if not batch:
        cam.show()
    cam = CameraDisplay(camgeom, stddev, ax=axes[1, 0], norm=norm, title=labels[1])
...
def write_error_page(tablename, pagesize):
    ...
    plt.text(
        0.5,
        0.5,
        "Sorry, no " + tablename + " to plot here!",
        fontsize=44,
...
def merge_dl1datacheck_files(file_list):
    ...
    merged_filename = run_to_datacheck_dl1_filename(file.tel_id, file.run, None, None)
    ...
    if "/dl1datacheck/pedestals" in first_file.root.dl1datacheck:
        pedestals = first_file.copy_node(
            "/dl1datacheck", name="pedestals", newparent=merged_file.root.dl1datacheck
        )
    else:
        pedestals = None

    if "/dl1datacheck/flatfield" in first_file.root.dl1datacheck:
        flatfield = first_file.copy_node(
            "/dl1datacheck", name="flatfield", newparent=merged_file.root.dl1datacheck
        )
    else:
        flatfield = None

    # the ones below are compulsory, an exception will be raised if not present:
    cosmics = first_file.copy_node(
        "/dl1datacheck", name="cosmics", newparent=merged_file.root.dl1datacheck
    )
    first_file.copy_node(
        "/dl1datacheck",
    ...
    for filename in file_list[1:]:
        ...
        if pedestals is not None:
            if "/dl1datacheck/pedestals" in file.root.dl1datacheck:
                ...
                logger.warning("Table pedestals is missing in file " + str(filename))
        if flatfield is not None:
            if "/dl1datacheck/flatfield" in file.root.dl1datacheck:
                flatfield.append(file.root.dl1datacheck.flatfield[:])
            else:
                logger.warning("Table flatfield is missing in file " + str(filename))
