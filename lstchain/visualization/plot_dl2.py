...
def plot_features(data, true_hadroness=False):
    ...
    plt.subplot(331)
    plt.hist(
        data[data[hadro] < 1]["log_mc_energy"],
        histtype="step",
        bins=100,
        label="Gammas",
    )
    plt.hist(
        data[data[hadro] > 0]["log_mc_energy"],
        histtype="step",
        bins=100,
        label="Protons",
    )
    ...
    plt.subplot(332)
    plt.hist(
        data[data[hadro] < 1]["disp_norm"], histtype="step", bins=100, label="Gammas"
    )
    plt.hist(
        data[data[hadro] > 0]["disp_norm"], histtype="step", bins=100, label="Protons"
    )
    plt.ylabel(r"# of events", fontsize=15)
    ...
    plt.subplot(333)
    plt.hist(
        data[data[hadro] < 1]["log_intensity"],
        histtype="step",
        bins=100,
        label="Gammas",
    )
    plt.hist(
        data[data[hadro] > 0]["log_intensity"],
        histtype="step",
        bins=100,
        label="Protons",
    )
    ...
    plt.subplot(334)
    plt.hist(
        dataforwl[dataforwl[hadro] < 1]["width"],
        histtype="step",
        bins=100,
        label="Gammas",
    )
    plt.hist(
        dataforwl[dataforwl[hadro] > 0]["width"],
        histtype="step",
        bins=100,
        label="Protons",
    )
    ...
    plt.subplot(335)
    plt.hist(
        dataforwl[dataforwl[hadro] < 1]["length"],
        histtype="step",
        bins=100,
        label="Gammas",
    )
    plt.hist(
        dataforwl[dataforwl[hadro] > 0]["length"],
        histtype="step",
        bins=100,
        label="Protons",
    )
    ...
    # r distribution
    plt.subplot(336)
    plt.hist(data[data[hadro] < 1]["r"], histtype="step", bins=100, label="Gammas")
    plt.hist(data[data[hadro] > 0]["r"], histtype="step", bins=100, label="Protons")
    plt.ylabel(r"# of events", fontsize=15)
    plt.xlabel(r"r (m)")
    ...
    plt.subplot(337)
    plt.hist(data[data[hadro] < 1]["psi"], histtype="step", bins=100, label="Gammas")
    plt.hist(data[data[hadro] > 0]["psi"], histtype="step", bins=100, label="Protons")
    plt.ylabel(r"# of events", fontsize=15)
    ...
    plt.subplot(338)
    plt.hist(data[data[hadro] < 1]["phi"], histtype="step", bins=100, label="Gammas")
    plt.hist(data[data[hadro] > 0]["phi"], histtype="step", bins=100, label="Protons")
    plt.ylabel(r"# of events", fontsize=15)
    ...
    plt.subplot(339)
    plt.hist(
        data[data[hadro] < 1]["time_gradient"],
        histtype="step",
        bins=100,
        label="Gammas",
    )
    plt.hist(
        data[data[hadro] > 0]["time_gradient"],
        histtype="step",
        bins=100,
        label="Protons",
    )
...
def energy_results(dl2_data, points_outfile=None, plot_outfile=None):
    ...
    ctaplot.resolution_per_energy(
        dl2_data.mc_energy, dl2_data.reco_energy, dl2_data.reco_energy
    )
    ctaplot.plot_energy_resolution(
        dl2_data.mc_energy, dl2_data.reco_energy, ax=axes[0, 0], bias_correction=False
    )
    ...
    ctaplot.plot_migration_matrix(
        dl2_data.mc_energy.apply(np.log10),
    ...
    if points_outfile:
        e_bins, e_res = ctaplot.energy_resolution_per_energy(
            dl2_data.mc_energy, dl2_data.reco_energy
        )
...
def write_energy_resolutions(
    outfile, e_bins, res, bias=None, overwrite=False, append=True
):
    """
...
def plot_disp(data, true_hadroness=False):
    ...
    plt.xlabel(
        r"$\\frac{disp\_norm_{gammas}-disp_{rec}}{disp\_norm_{gammas}}$", fontsize=15
    )
    ...
    plt.subplot(223)
    theta2 = (gammas["src_x"] - gammas["reco_src_x"]) ** 2 + (
        gammas["src_y"] - gammas["src_y"]
    ) ** 2

    plt.hist(theta2, bins=100, range=[0, 0.1], histtype="step")
...
def plot_pos(data, true_hadroness=False):
    ...
    plt.subplot(221)
    plt.hist2d(trueXprot, trueYprot, bins=nbins, label="Protons", range=ran)
    plt.colorbar()
    ...
    plt.subplot(222)
    plt.hist2d(trueX, trueY, bins=nbins, label="Gammas", range=ran)
    plt.colorbar()
    ...
    plt.subplot(223)
    plt.hist2d(recXprot, recYprot, bins=nbins, label="Protons", range=ran)
    plt.colorbar()
...
def plot_models_features_importances(
    path_models, config_file=None, axes=None, **kwargs
):
    """
...
def plot_roc_gamma(dl2_data, energy_bins=None, ax=None, **kwargs):
    ...
    if energy_bins is None:
        ax = ctaplot.plot_roc_curve_gammaness(
            dl2_data.mc_type, dl2_data.gammaness, ax=ax, **kwargs
        )
    else:
        ax = ctaplot.plot_roc_curve_gammaness_per_energy(
            dl2_data.mc_type,
            dl2_data.gammaness,
            dl2_data.mc_energy,
...
def plot_energy_resolution(
    dl2_data, ax=None, bias_correction=False, cta_req_north=False, **kwargs
):
    """
    ...
    ax.grid(which="both")
    if cta_req_north:
        ax = ctaplot.plot_energy_resolution_cta_requirement(
            "north", ax=ax, color="black"
        )
    return ax
...
def plot_angular_resolution(
    ...
    ax.grid(which="both")
    if cta_req_north:
        ax = ctaplot.plot_angular_resolution_cta_requirement(
            "north", ax=ax, color="black"
        )
...
def plot_wobble(source_position, n_points, ax=None):
    ...
    marker_size = 20
    color_map_name = (
        "Set1"  # https://matplotlib.org/gallery/color/colormap_reference.html
    )
    colors = get_cmap(color_map_name).colors
    ax.set_prop_cycle(color=colors)

    rotation_angle = 360.0 / n_points
    labels = [
        "Source",
    ...
    for off_point in range(n_points):
        first_point = tuple(
            rotate(
                list(zip(source_position[0].to_value(), source_position[1].to_value()))[
                    0
                ],
...
def plot_1d_excess(
    ...
    ax.annotate(
        text=rf"Significance Li&Ma = {lima_significance:.2f} $\sigma$\n",
        xy=(np.max(hists[0][1] / 4), np.max(hists[0][0] / 6 * 5)),
        size=20,
        color="r",
    )
