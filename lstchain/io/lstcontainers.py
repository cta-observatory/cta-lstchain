...
class DL1ParametersContainer(Container):
    ...
    disp_norm = Field(u.Quantity(np.nan, u.m), 'disp_norm [m]', unit=u.m)
    disp_dx = Field(u.Quantity(np.nan, u.m), 'disp_dx [m]', unit=u.m)
    disp_dy = Field(u.Quantity(np.nan, u.m), 'disp_dy [m]', unit=u.m)
    disp_angle = Field(u.Quantity(np.nan, u.rad), 'disp_angle [rad]', unit=u.rad)
    disp_sign = Field(np.nan, 'disp_sign')
    disp_miss = Field(u.Quantity(np.nan, u.m), 'disp_miss [m]', unit=u.m)
    src_x = Field(u.Quantity(np.nan, u.m), 'source x coordinate in camera frame', unit=u.m)
    src_y = Field(u.Quantity(np.nan, u.m), 'source y coordinate in camera frame', unit=u.m)
