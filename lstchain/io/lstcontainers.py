...
class DL1ParametersContainer(Container):
    ...
    phi = Field(Angle(np.nan, u.rad), "polar coordinate of centroid", unit=u.rad)
    length = Field(
        u.Quantity(np.nan, u.deg), "RMS spread along the major-axis", unit=u.deg
    )
    width = Field(
        u.Quantity(np.nan, u.deg), "RMS spread along the minor-axis", unit=u.deg
    )
    ...
    leakage_intensity_width_1 = Field(
        np.float32(np.nan),
        "Fraction of intensity in outermost pixels",
        dtype=np.float32,
    )
    leakage_intensity_width_2 = Field(
        np.float32(np.nan),
        "Fraction of intensity in two outermost " "rings of pixels",
        dtype=np.float32,
    )
    leakage_pixels_width_1 = Field(
        np.nan, "Fraction of signal pixels that are " "border pixels"
    )
    leakage_pixels_width_2 = Field(
        np.nan,
    ...
    concentration_cog = Field(
        np.nan, "Fraction of intensity in three pixels " "closest to the cog"
    )
    concentration_core = Field(np.nan, "Fraction of intensity inside hillas " "ellipse")
    concentration_pixel = Field(np.nan, "Fraction of intensity in brightest " "pixel")
    ...
    mc_core_distance = Field(
        None, "Distance from the impact point to the telescope", unit=u.m
    )
    ...
    def fill_mc(self, event, tel_pos):
        ...
        try:
            ...
            distance = np.sqrt(
                (shower.core_x - tel_pos[0]) ** 2 + (shower.core_y - tel_pos[1]) ** 2
            )
    ...
    def get_features(self, features_names):
        return np.array(
            [
                self[k].value if isinstance(self[k], Quantity) else self[k]
...
class ThrownEventsHistogram(Container):
    ...
    bins_core_dist = Field(
        None, "array of core-distance bin lower edges, as in np.histogram"
    )
