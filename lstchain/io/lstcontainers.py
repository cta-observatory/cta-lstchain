...
class DL1ParametersContainer(Container):
    ...
    log_intensity = Field(nan, 'log of total intensity (size)')

    x = Field(nan, 'centroid x coordinate', unit=u.m)
    y = Field(nan, 'centroid x coordinate', unit=u.m)
    r = Field(nan, 'radial coordinate of centroid', unit=u.m)
    phi = Field(nan, 'polar coordinate of centroid', unit=u.rad)
    length = Field(nan, 'RMS spread along the major-axis', unit=u.m)
    width = Field(nan, 'RMS spread along the minor-axis', unit=u.m)
    psi = Field(nan, 'rotation angle of ellipse', unit=u.rad)
    skewness = Field(nan, 'measure of the asymmetry')
    kurtosis = Field(nan, 'measure of the tailedness')
    disp_norm = Field(nan, 'disp_norm [m]', unit=u.m)
    disp_dx = Field(nan, 'disp_dx [m]', unit=u.m)
    disp_dy = Field(nan, 'disp_dy [m]', unit=u.m)
    disp_angle = Field(nan, 'disp_angle [rad]', unit=u.rad)
    disp_sign = Field(nan, 'disp_sign')
    disp_miss = Field(nan, 'disp_miss [m]', unit=u.m)
    src_x = Field(nan, 'source x coordinate in camera frame', unit=u.m)
    src_y = Field(nan, 'source y coordinate in camera frame', unit=u.m)
    time_gradient = Field(nan, 'Time gradient in the camera')
    intercept = Field(nan, 'Intercept')
    leakage = Field(nan, 'Leakage')
    n_islands = Field(nan, 'Number of Islands')
    alt_tel = Field(nan, 'Telescope altitude pointing', unit=u.rad)
    az_tel = Field(nan, 'Telescope azimuth pointing', unit=u.rad)

    obs_id = Field(nan, 'Observation ID')
    event_id = Field(nan, 'Event ID')
    gps_time = Field(nan, 'GPS time event trigger')
    dragon_time = Field(nan, 'Dragon time event trigger')
    ucts_time = Field(nan, 'UCTS time event trigger')
    tib_time = Field(nan, 'TIB time event trigger')

    mc_energy = Field(nan, 'Simulated Energy', unit=u.TeV)
    log_mc_energy = Field(nan, 'log of simulated energy/GeV')
    mc_alt = Field(nan, 'Simulated altitude', unit=u.rad)
    mc_az = Field(nan, 'Simulated azimuth', unit=u.rad)
    mc_core_x = Field(nan, 'Simulated impact point x position', unit=u.m)
    mc_core_y = Field(nan, 'Simulated impact point y position', unit=u.m)
    mc_h_first_int = Field(nan, 'Simulated first interaction height', unit=u.m)
    mc_type = Field(nan, 'Simulated particle type')
    mc_az_tel = Field(nan, 'Telescope MC azimuth pointing', unit=u.rad)
    mc_alt_tel = Field(nan, 'Telescope MC altitude pointing', unit=u.rad)
    mc_x_max = Field(nan, "MC Xmax value", unit=u.g / (u.cm ** 2))
    mc_core_distance = Field(nan, "Distance from the impact point to the telescope", unit=u.m)
    mc_shower_primary_id = Field(nan, "MC shower primary ID 0 (gamma), 1(e-),"
                                    "2(mu-), 100*A+Z for nucleons and nuclei,"
                                    "negative for antimatter.")

    hadroness = Field(nan, "Hadroness")
    wl = Field(nan, "width/length")

    tel_id = Field(nan, "Telescope Id")
    tel_pos_x = Field(nan, "Telescope x position in the ground")
    tel_pos_y = Field(nan, "Telescope y position in the ground")
    tel_pos_z = Field(nan, "Telescope z position in the ground")
...
class ExtraImageInfo(Container):
    ...
    trigger_type = Field(nan, "trigger type")
    trigger_time = Field(nan, "trigger time")
    num_trig_pix = Field(nan, "Number of trigger groups (sectors) listed")
    trig_pix_id = Field(nan, "pixels involved in the camera trigger")
...
class ThrownEventsHistogram(Container):
    ...
    bins_energy = Field(nan, 'array of energy bin lower edges, as in np.histogram')
    bins_core_dist = Field(nan, 'array of core-distance bin lower edges, as in np.histogram')
    histogram = Field(nan, "array of histogram entries, size (n_bins_x, n_bins_y)")
