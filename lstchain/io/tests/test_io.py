...
@pytest.fixture
def merged_h5file(tmp_path, simulated_dl1):
    ...
    smart_merge_h5files(
        [simulated_dl1["file"], simulated_dl1["file"]],
...
@pytest.mark.run(after='test_r0_to_dl1')
def test_read_simu_info_hdf5(simulated_dl1):
    from lstchain.io.io import read_simu_info_hdf5
    mcheader = read_simu_info_hdf5(simulated_dl1["file"])
...
@pytest.mark.run(after='test_r0_to_dl1')
def test_trigger_type_in_dl1_params(simulated_dl1):
    from lstchain.io.io import dl1_params_lstcam_key
    params = pd.read_hdf(simulated_dl1["file"], key=dl1_params_lstcam_key)
    assert 'trigger_type' in params.columns


@pytest.mark.run(after='test_r0_to_dl1')
def test_read_subarray_description(mc_gamma_testfile, simulated_dl1):
    from lstchain.io.io import read_subarray_description
    from ctapipe.io import EventSource
    source = EventSource(mc_gamma_testfile)
    dl1_subarray = read_subarray_description(simulated_dl1["file"])
    dl1_subarray.peek()
    dl1_subarray.info()
