...
def main():
    ...
    for file in files:
        ...
        if datatables[1] is not None:
            ...
            pedestals["charge_mean"].extend(table.col("charge_mean").mean(axis=1))
            pedestals["charge_stddev"].extend(table.col("charge_stddev").mean(axis=1))

        if datatables[2] is not None:
            table = a.root.dl1datacheck.flatfield
            flatfield["charge_mean"].extend(table.col("charge_mean").mean(axis=1))
            flatfield["charge_stddev"].extend(table.col("charge_stddev").mean(axis=1))
        ...
        if datatables[1] is not None:
            ...
            charge_mean = (
                np.sum(table.col("charge_mean") * nevents[:, None], axis=0)
                / events_in_run
            )
            ...
            charge_stddev = np.sqrt(
                np.sum((table.col("charge_stddev") ** 2) * nevents[:, None], axis=0)
                / events_in_run
            )
        ...
        if datatables[2] is not None:
            ...
            time_mean = (
                np.sum(table.col("time_mean") * nevents[:, None], axis=0)
                / events_in_run
            )
            ...
            time_stddev = np.sqrt(
                np.sum((table.col("time_stddev") ** 2) * nevents[:, None], axis=0)
                / events_in_run
            )
            # Store the pixel-averaged FF time std dev:
            runsummary["ff_time_stddev"].extend([np.nanmean(time_stddev)])

            rel_time_stddev = np.sqrt(
                np.sum(
                    (table.col("relative_time_stddev") ** 2) * nevents[:, None], axis=0
                )
                / events_in_run
            )
            runsummary["ff_rel_time_stddev"].extend([np.nanmean(rel_time_stddev)])
            ...
            pixwise_runsummary["ff_pix_rel_time_mean"].extend([numpixels * [np.nan]])
            pixwise_runsummary["ff_pix_rel_time_stddev"].extend([numpixels * [np.nan]])
        ...
        for subrun in subruns:
            ...
            contained_mu = df_muons[
                (df_muons["ring_containment"] > 0.99) & (df_muons["size_outside"] < 1.0)
            ]
        ...
        if empty_files > 0:
            print(
                "   Run {0:d} had {1:d} subruns with no valid muon rings!".format(
                    runnumber, empty_files
                )
            )

        # fill the runsummary muons part:
        if contained_mu_wholerun is not None:
            runsummary["num_contained_mu_rings"].extend([num_contained_mu_rings_in_run])
            ...
            runsummary["mu_hg_peak_sample_mean"].extend(
                [contained_mu_wholerun["hg_peak_sample"].mean()]
            )
            runsummary["mu_hg_peak_sample_stddev"].extend(
                [contained_mu_wholerun["hg_peak_sample"].std()]
            )
            runsummary["mu_lg_peak_sample_mean"].extend(
                [contained_mu_wholerun["lg_peak_sample"].mean()]
            )
            runsummary["mu_lg_peak_sample_stddev"].extend(
                [contained_mu_wholerun["lg_peak_sample"].std()]
            )
        else:
...
def plot(filename="longterm_dl1_check.h5", tel_id=1):
    ...
    bokeh_output_file(
        Path(filename).with_suffix(".html"), title="LST1 long-term DL1 data check"
    )

    run_titles = []
    for i, run in enumerate(file.root.pixwise_runsummary.col("runnumber")):
        date = pd.to_datetime(
            file.root.pixwise_runsummary.col("time")[i], origin="unix", unit="s"
        )
        run_titles.append(
            "Run {0:05d}, {date}".format(run, date=date.strftime("%b %d %Y %H:%M:%S"))
        )
    ...
    fig_muring_rates = show_graph(
        ...
        ey=np.sqrt(runsummary["num_contained_mu_rings"]) / runsummary["elapsed_time"],
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    ...
    row1 = [fig_altitude]
    grid0b = gridplot(
        [row1], sizing_mode=None, plot_width=pad_width, plot_height=pad_height
    )
    page0b.child = grid0b
    ...
    for item in file.root.pixwise_runsummary.col("cosmics_pix_fraction_pulses_above10"):
        pulse_fraction_above_10.append(item)
    for item in file.root.pixwise_runsummary.col("cosmics_pix_fraction_pulses_above30"):
        pulse_fraction_above_30.append(item)

    row1 = show_camera(
        np.array(pulse_fraction_above_10),
        engineering_geom,
        pad_width,
        pad_height,
        "Cosmics, fraction of >10pe pulses",
        run_titles,
    )
    row2 = show_camera(
        np.array(pulse_fraction_above_30),
        engineering_geom,
        pad_width,
        pad_height,
        "Cosmics, fraction of >30pe pulses",
        run_titles,
    )
    ...
    pad_width = 550
    pad_height = 280
    fig_mu_effi = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
        y=runsummary["mu_effi_mean"],
        xlabel="date",
        ylabel="telescope efficiency from mu-rings",
        ey=runsummary["mu_effi_stddev"] / np.sqrt(runsummary["num_contained_mu_rings"]),
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    fig_mu_effi.y_range = Range1d(0.0, 1.1 * np.max(runsummary["mu_effi_mean"]))

    fig_mu_width = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
    ...
    fig_mu_width.y_range = Range1d(0.0, 1.1 * np.max(runsummary["mu_width_mean"]))
    ...
    fig_mu_intensity.y_range = Range1d(
        0.0, 1.1 * np.max(runsummary["mu_intensity_mean"])
    )
    ...
    fig_mu_hg_peak.y_range = Range1d(0.0, 38.0)
    ...
    fig_mu_lg_peak.y_range = Range1d(0.0, 38.0)
    ...
    fig_ped.y_range = Range1d(0.0, 1.1 * np.max(runsummary["ped_charge_mean"]))

    fig_ped_stddev = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
        y=runsummary["ped_charge_stddev"],
        xlabel="date",
        ylabel="Camera-averaged pedestal charge std " "dev (pe/pixel)",
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    fig_ped_stddev.y_range = Range1d(0.0, 1.1 * np.max(runsummary["ped_charge_stddev"]))

    frac = runsummary["num_pedestals_after_cleaning"] / runsummary["num_pedestals"]
    ...
    fig_flatfield.y_range = Range1d(0.0, 1.1 * np.max(runsummary["ff_charge_mean"]))

    fig_ff_stddev = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
        y=runsummary["ff_charge_stddev"],
        xlabel="date",
        ylabel="Cam-averaged FF Q std " "dev (pe/pixel)",
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    fig_ff_stddev.y_range = Range1d(0.0, 1.1 * np.max(runsummary["ff_charge_stddev"]))

    fig_ff_time = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
    ...
    fig_ff_time_std = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
        y=runsummary["ff_time_stddev"],
        xlabel="date",
        ylabel="Cam-averaged FF t std " "dev (ns)",
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    fig_ff_rel_time_std = show_graph(
        x=pd.to_datetime(runsummary["time"], origin="unix", unit="s"),
        y=runsummary["ff_rel_time_stddev"],
        xlabel="date",
        ylabel="Cam-averaged FF " "rel. pix t std dev (ns)",
        xtype="datetime",
        ytype="linear",
        point_labels=run_titles,
    )
    fig_ff_rel_time_std.y_range = Range1d(
        0.0, np.max([1.0, runsummary["ff_rel_time_stddev"].max()])
    )
    ...
    tabs = Tabs(tabs=[page0, page0b, page1, page2, page3, page4, page5, page6, page7])
...
def show_graph(
    ...
    if eylow is not None or eyhigh is not None:
        ...
        error_bars = Whisker(
            source=source_error, base="base", lower="lower", upper="upper"
        )
    ...
    if point_labels is not None:
        fig.add_tools(
            HoverTool(
                tooltips=[("value", "@y"), ("point id", "@point_labels")],
                mode="mouse",
