...
def main():
    ...
    if (
        "alt_tel" in data.columns
        and "az_tel" in data.columns
        ...
        if np.isfinite(data.alt_tel).any() and np.isfinite(data.az_tel).any():
            ...
            data.alt_tel = -np.pi / 2.0
            data.az_tel = -np.pi / 2.0
    ...
    output_file = os.path.join(
        args.output_dir, os.path.basename(args.input_file).replace("dl1", "dl2")
    )
        ...
        with open_file(output_file, "a") as h5out:
            ...
            for k in dl1_keys:
                ...
                if path not in h5out:
                    ...
                    g = h5out.create_group(grouppath, groupname, createparents=True)
                else:
    ...
    if not config["source_dependent"]:
        ...
        write_dataframe(
            pd.concat(dl2_srcdep_dict, axis=1),
            output_file,
            dl2_params_src_dep_lstcam_key,
        )
