#!/usr/bin/env python3
__doc__ = """
Run the DL1 to DL2 step interpolating results from a set of single point Random forests :
Pipeline for the reconstruction of Energy, disp and gamma/hadron separation of events stored in a DL1 file.
It takes DL1 file(s) and trained Random Forests as input and outputs DL2 data file(s).
Run lstchain_dl1_to_dl2_node_interpolation --help to see the options.
"""
...
import lstchain.reco.dl1_to_dl2
...
parser.add_argument('--path-models', '-p',
                    ...
                    type=Path,
                    dest='path_models',
                    help='Path where to find the node folders containing the trained RF',
...
logging.getLogger('lstchain').setLevel(parser.parse_args().loglevel.upper())

def apply_to_file(filename, model_path, output_dir, config):
    ...
    if not config['source_dependent']:
        ...
        dl2 = dl1_to_dl2.apply_models_node_interpolation(data,
                                                         model_node_loc=model_path,
    ...
    if config['source_dependent']:
        ...
        for i, k in enumerate(srcdep_assumed_positions):
            ...
            dl2_df = dl1_to_dl2.apply_models_node_interpolation(data_with_srcdep_param,
                                                                model_node_loc=model_path,
                                                                effective_focal_length=effective_focal_length,
                                                                custom_config=config)
...
def main():
    ...
    for filename in args.input_files:
        apply_to_file(filename, args.path_models, args.output_dir, config)
