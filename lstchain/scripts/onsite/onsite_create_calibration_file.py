...
parser = argparse.ArgumentParser(
    description="Create flat-field calibration files",
...
optional.add_argument(
    "-v", "--version", help="Version of the production", type=int, default=0
)
optional.add_argument(
    "-s",
    "--statistics",
...
optional.add_argument(
    "--default_time_run",
    help="If 0 time calibration is calculated otherwise create a link to the give run time calibration",
    type=int,
    default="1625",
)
optional.add_argument(
    "--ff_calibration",
    help="Perform the charge calibration (yes/no)",
    type=str,
    default="yes",
)
optional.add_argument("--tel_id", help="telescope id. Default = 1", type=int, default=1)
optional.add_argument(
    "--sub_run", help="sub-run to be processed. Default = 0", type=int, default=0
)
optional.add_argument(
    "--min_ff",
    help="Min FF intensity cut in ADC. Default = 4000",
    type=float,
    default=4000,
)
optional.add_argument(
    "--max_ff",
    help="Max FF intensity cut in ADC. Default = 12000",
    type=float,
    default=12000,
)
...
def main():
    ...
    try:
        ...
        config_file = os.path.join(
            os.path.dirname(__file__), "../../data/onsite_camera_calibration_param.json"
        )
        ...
        if default_time_run is 0:
            ...
            cmd = (
                f"lstchain_data_create_time_calibration_file  --input-file {input_file} "
        ...
        if ff_calibration == "yes":
            # run lstchain script
            cmd = (
                f"lstchain_create_calibration_file "
            ...
            calib.plot_all(
                calib.ped_data, calib.ff_data, calib.calib_data, run, plot_file
            )
