...
#  This script was validated with ctapipe(0.7.0.post171), cta-lstchain(March 2) and ctapipe_io_lst(Dec. 17) in April 14.
#  This script creates the sampling intervals for each DRS4 cell of all pixels only high gain or low gain, and the result outputs as fits file.
#  This scripts is short version of sampling_interval_creation.py. If you want to create earlier than sampling_interval_creation.py, please use this script.
# *Caution* The interval table creation is needed 10^6 events (or more) of test pulse data to use the DRS4 timing-calibration. But this script can create ~53000 events. You need to run some processes.
# Important 2.: This script subtracts pedestal and corrects the spikes & time-lapse before DRS4 timing-calibration.
# Important 3.: please set the output file name with one pattarn such as below and you need to merge the output files.
#  ex) RunXXX_interval_HG_0.fits, RunXXX_interval_HG_1.fits, ...
...
parser = argparse.ArgumentParser(
    usage='$python sampling_interval_creation.py --input_file /.../LST.1.1.RunXXX.0000.fits.fz --output_file sampling_interval_RunXXX_HG.fits --pedestal_file pedestal_RunYYY.fits --gain_selection 0 --num_events 50000',
...
parser.add_argument('-n', '--num_events', help='Set the number of events you want to use. The default is "None", i.e. all events(~53000) are used.', 
...
reader = event_source(args.path_to_data)
for event in reader:
...
sampling_interval = (peak_counts/num_loop)*n_cell
