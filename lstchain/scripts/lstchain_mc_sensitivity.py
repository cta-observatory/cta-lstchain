...
def main():
    ...
    plot_utils.plot_Crab_SED(
        ax, 10, 50, 5e4, linestyle="--", label="10% Crab"
    )  # Energy in GeV
    plot_utils.plot_Crab_SED(
        ax, 1, 50, 5e4, linestyle=":", label="1% Crab"
    )  # Energy in GeV
    plot_utils.plot_sensitivity(
        energy, sensitivity, ax, color="orange", label="Sensitivity real data"
    )
    plot_utils.plot_sensitivity(
        energy, mc_sensitivity, ax, color="green", label="Sensitivity MC gammas"
    )
    ...
    ctaplot.plot_energy_resolution(
        events[events.mc_type == 0].mc_energy, events[events.mc_type == 0].reco_energy
    )
    ...
    total_number_of_events = (
        gamma_ps_simu_info.num_showers
        * gamma_ps_simu_info.shower_reuse
        * ntelescopes_gamma
    )
    spectral_index = gamma_ps_simu_info.spectral_index
    area = (
        gamma_ps_simu_info.max_scatter_range.value
        - gamma_ps_simu_info.min_scatter_range.value
    ) ** 2 * np.pi
