...
test_calib_path = (
    test_data / "real/calibration/20200218/v05/calibration.Run2006.0000.hdf5"
)
test_drs4_pedestal_path = (
    test_data / "real/calibration/20200218/v05/drs4_pedestal.Run2005.0000.fits"
)
test_time_calib_path = (
    test_data / "real/calibration/20200218/v05/time_calibration.Run2006.0000.hdf5"
)
test_drive_report = (
...
def test_r0_to_dl1(tmp_path, mc_gamma_testfile):
    ...
    r0_to_dl1(
        infile, custom_config=standard_config, output_filename=tmp_path / "dl1_gamma.h5"
    )
...
def test_r0_to_dl1_observed(tmp_path):
    ...
    lst_event_source["LSTR0Corrections"]["drs4_pedestal_path"] = test_drs4_pedestal_path
    lst_event_source["LSTR0Corrections"]["calibration_path"] = test_calib_path
    lst_event_source["LSTR0Corrections"][
        "drs4_time_calibration_path"
    ] = test_time_calib_path
...
def test_build_models(simulated_dl1_file, rf_models):
    ...
    reg_energy, reg_disp, cls_gh = build_models(
        infile, infile, custom_config=standard_config, save_models=False
    )
...
def test_apply_models(simulated_dl1_file, simulated_dl2_file, rf_models):
    ...
    dl2 = apply_models(
        dl1, reg_cls_gh, reg_energy, reg_disp, custom_config=standard_config
    )
...
def test_change_frame_camera_sky():
    ...
    focal_length = 5 * u.m
    pointing_alt = np.pi / 3.0 * u.rad
    pointing_az = 0.0 * u.rad

    sky_pos = camera_to_altaz(x, y, focal_length, pointing_alt, pointing_az)
    cam_pos = sky_to_camera(
        sky_pos.alt, sky_pos.az, focal_length, pointing_alt, pointing_az
    )
