...
def test_generic():
    ...
    assert run_info_from_filename("/foo/bar/LST-1.2.Run01920.0001") == Run(
        1, 1920, 1, 2
    )
...
def test_dl1_to_filename():
    ...
    assert (
        run_to_dl1_filename(tel_id=1, run=1920, subrun=2)
        == "dl1_LST-1.Run01920.0002.h5"
    )
    assert (
        run_to_dl1_filename(tel_id=1, run=1920, subrun=3, stream=2)
...
def test_dl2_to_filename():
    ...
    assert (
        run_to_dl2_filename(tel_id=1, run=1920, subrun=2)
        == "dl2_LST-1.Run01920.0002.h5"
    )
    assert (
        run_to_dl2_filename(tel_id=1, run=1920, subrun=3, stream=2)
...
def test_r0_to_filename():
    ...
    assert (
        run_to_r0_filename(tel_id=1, run=1920, subrun=3, stream=2)
...
def test_muon_filename():
    ...
    assert (
        run_to_muon_filename(tel_id=1, run=2, subrun=3)
        == "muons_LST-1.Run00002.0003.fits.gz"
    )

    assert (
        run_to_muon_filename(tel_id=1, run=2, subrun=3, gzip=False)
        == "muons_LST-1.Run00002.0003.fits"
    )

    assert (
        run_to_muon_filename(tel_id=1, run=2, subrun=3, stream=4)
...
def test_r0_to_dl1_filename():
    ...
    assert (
        str(r0_to_dl1_filename("LST1_custom.test.fits.fz")) == "dl1_LST1_custom.test.h5"
    )
