            ...
            with EventSource(path,parent=self) as eventsource:
                ...
                for count, event in enumerate(tqdm(eventsource)):
                    ...
                    if event.calibration.tel[tel_id].dl1.time_shift is None:
                        if 'pixel_time_shift' in event.r1.tel[tel_id].meta:
                            sg = event.r1.tel[tel_id].selected_gain_channel
                            if sg is None:
                                event.calibration.tel[tel_id].dl1.time_shift = event.r1.tel[tel_id].meta['pixel_time_shift']
                            else:
                                event.calibration.tel[tel_id].dl1.time_shift = np.zeros((N_GAINS, N_PIXELS))
                                event.calibration.tel[tel_id].dl1.time_shift[sg, PIXEL_INDEX] = event.r1.tel[tel_id].meta['pixel_time_shift'][0, PIXEL_INDEX]
                    ...
                    if (new_ff and new_ped):
                        ...
                        if 'pixel_time_shift' not in event.r1.tel[tel_id].meta:
