...
def setup_logging(verbosity=1):
    ...
    fh = logging.FileHandler(
        f"/tmp/lstchain-postDL2-{getpass.getuser()}_{time.time()}.log"
    )
...
def analyze_wobble(config):
    ...
    LOGGER.info(
        "Running wobble analysis with %s off-source observation points", n_points
    )
    LOGGER.info("Analyzing runs %s", config["analysis"]["runs"])
    observation_time, data = merge_dl2_runs(
        config["input"]["data_tag"],
    ...
    named_datasets.append(
        ("ON data", np.array(compute_theta2(selected_data, true_source_position)), 1)
    )
    ...
    for off_point in range(1, n_points):
        ...
        named_datasets.append(
            (
                ...
                np.array(compute_theta2(t_off_data, true_source_position)),
                1,
            )
        )
        n_off += np.sum(named_datasets[-1][1] < theta2_cut)

    stat = WStatCountsStatistic(n_on, n_off, 1.0 / (n_points - 1))
    ...
    LOGGER.info("ON/OFF observation time ratio %s", 1.0 / (n_points - 1))
    LOGGER.info("Excess is %s", lima_excess)
    LOGGER.info("Li&Ma significance %s", lima_significance)
    plotting.plot_1d_excess(
        named_datasets, lima_significance, r"$\theta^2$ [deg$^2$]", theta2_cut, ax2
    )
